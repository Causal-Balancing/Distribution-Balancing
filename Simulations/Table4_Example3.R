...
for(n_level in n_set){
  ...
  result <- foreach(k = 1:reps, .combine = 'rbind',
      ...
      my_forest_option = list(nodesize=10,mtry=2, ntree=1000, na.action=na.omit, replace=TRUE)
      ...
      fit_entropy <- fun_CCDF_balance_new(y,D,X,taus,fun_obj='entropy', 
                                         num_localCCDF=0,num_moment=1,alpha=0)
      ...
      fit_ccdf_full <- fun_CCDF_balance_old(y, D, X,taus,method_ccdf = method_ccdf,
                                              ...
                                              est_ccdf_normlize=FALSE,alpha=0)
      ...
      fit_ccdf_balance <- fun_CCDF_balance_new(y, D, X, taus,K=2, method_ccdf=method_ccdf,
                                                 ...
                                                 num_moment=c(0,1), 
                                                 moments_normlize = FALSE,
                                                 est_ccdf_normlize=FALSE,alpha=0.01)
      ...
      #qte_our_moment0_fold1 <- fit_ccdf_balance$final_results$moment_0$est_QTE_cdf_folds[1,]
      #qte_our_moment1_fold1 <- fit_ccdf_balance$final_results$moment_1$est_QTE_cdf_folds[1,]
        
      #qte_our_moment0_fold2 <- fit_ccdf_balance$final_results$moment_0$est_QTE_cdf_folds[2,]
      #qte_our_moment1_fold2 <- fit_ccdf_balance$final_results$moment_1$est_QTE_cdf_folds[2,]
    ...
    names0 <- c('DIQ', 'PLE-IPW','Energy', 'Kernel','LDML')

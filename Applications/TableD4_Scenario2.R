...
datatest <- read.csv('Applications/data_scenario2.csv')
datatest<-datatest[,-1]
...
save_dir <- paste0("A_Scenario2_", timestamp)
...
for(n_level in n_set){
  ...
  result <- foreach(k = 1:reps, .combine = 'rbind',
                    .packages = c('quantreg', 'drf', 'quantregForest', 'osqp','ATE.ncb')) %dopar% {
                      ...
                      fit_ccdf_full <- fun_CCDF_balance_old(y, D, X,taus,method_ccdf = method_ccdf,
                                                            ...
                                                            num_quantreg_grid=200,epsilon=0.05,delta=0)
  ...
  file_name <- paste0(sprintf("Scenario2_n=%d_", n_level),timestamp,".csv")
...
file_name2 <- paste0("all_Scenario2_",timestamp,".csv")

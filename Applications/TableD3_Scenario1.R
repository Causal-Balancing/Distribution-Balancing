...
datatest <- read.csv('Applications/data_scenario1.csv')
datatest<-datatest[,-1]
colnames(datatest)<-c('y','t',paste("X.",1:2,sep=""),'y0','y1')
...
save_dir <- paste0("A_Scenario1_", timestamp)
...
for(n_level in n_set){
  ...
  result <- foreach(k = 1:reps, .combine = 'rbind',
      ...
      fit_entropy <- fun_CCDF_balance_new(y,D,X,taus,fun_obj='entropy', 
                                         ...
                                         moments_normlize = TRUE,alpha=0,delta = 0)
      ...
      for(c_level in c_set){
        ...
        fit_ccdf_full <- fun_CCDF_balance_old(y, D, X,taus,method_ccdf = method_ccdf,
                                              ...
                                              kernel_type=1, 
                                              bandwidth_c=bandwidth_c,delta=0
        )
        ...
        fit_ccdf_balance <- fun_CCDF_balance_new(y, D, X, taus,K=2, method_ccdf=method_ccdf,
                                                 ...
                                                 est_ccdf_normlize=TRUE, 
                                                 kernel_type=1, 
                                                 bandwidth_c=bandwidth_c,delta=0.01)
    ...
    file_name <- paste0(sprintf("Scenario1_n=%d_", n_level),timestamp,".csv")
...
file_name2 <- paste0("all_Scenario1_",timestamp,".csv")
